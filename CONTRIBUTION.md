...
existing [Issues](https://github.com/KRTirtho/spotube/issues) that might help you. In case you have
...
- Provide project and platform versions (flutter, dart, pub, linux etc..), depending on what seems
  relevant.
...
- Determine if your bug is really a bug and not an error on your side e.g. using incompatible
  ...
  the [documentation](https://github.com/KRTirtho/spotube#readme). If you are looking for support,
...
- Open an [Issue](https://github.com/KRTirtho/spotube/issues/new). (Since we can't be sure at this
  point whether it is a bug or not, we ask you not to talk about a bug yet and not to label the
  issue.)
...
This section guides you through submitting an enhancement suggestion for Spotube, **including
...
- Make sure that you are using the latest version.
- Read the [documentation](https://github.com/KRTirtho/spotube#readme) carefully and find out if the
  functionality is already covered, maybe by an individual configuration.
- Perform a [search](https://github.com/KRTirtho/spotube/issues) to see if the enhancement has
...
- **Explain why this enhancement would be useful** to most Spotube users. You may also want to point
...
Do debugging/testing/build etc then submit to us with PR against the development branch (dev) &
...
Then, you can start translating the app by following these steps:

- Do all the steps in [Your First Code Contribution](#your-first-code-contribution)
- Make sure application starts in debug mode
- Now, in `lib/l10n/app_<2-letter code of your language>.arb` (create if not exists) add necessary
  translations
  > (You can follow the `lib/l10n/app_en.arb` for reference)
- If you're adding missing translations, you can check the `/untranslated_messages.json` file to see
  which messages are missing in your native locale
- If you added entirely new translations:
    - Add `const Locale('<2-letter language code>', '<2-letter ISO country code>')` in
      `lib/l10n/l10n.dart`'s `static final all = [...]` variable
    - Uncomment the Map entry of your locale from `lib/collections/language_codes.dart`'s
      `static final Map isoLangs = {` variable
- Now restart (hot restart if running already) the app in debug mode & go to "Settings" > "
  Language" & see if your locale shows up
